...
def main(mv_file, vid_file, save_dir):
    ...
    for f in range(15,len(mv_file), 5):
        ...
        elif np.all(left_arm_mvmt[f:f+5]>=0) and np.all(left_arm_mvmt[f:f+5]<0.1):
        ...
        elif np.all(right_arm_mvmt[f:f+5]>=0) and np.all(right_arm_mvmt[f:f+5]<0.1):
        ...
        elif np.all(head_mvmt[f:f+5]>=0) and np.all(head_mvmt[f:f+5]<0.1):
