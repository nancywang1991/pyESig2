...
def main(vid_folder, save_folder, disconnect_fldr, sbj_ids, days):
   for sbj_id in sbj_ids:
       for day in days:
            disconnect_file= "%s/%s_%s.txt" %(disconnect_fldr, sbj_id, day)
            if os.path.isfile(disconnect_file):
                ...
                vid_count = 0
                cur_time = start_time
                vid_name = "%s/%s/%s_%s/%s_%s_%04i.avi" %(vid_folder, sbj_id, sbj_id, day, sbj_id, day, vid_count)
                with open("%s/%s_%s.csv" %(save_folder, sbj_id, day), "wb") as csvfile:
                   ...
                   while os.path.exists(vid_name):
                       ...
                       vid_len -= timedelta(seconds=0.000041*vid_len.total_seconds())
                ...
                pickle.dump(result, open("%s/%s_%s.p" %(save_folder, sbj_id, day), "wb"))
