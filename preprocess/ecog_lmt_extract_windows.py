...
def transform_file(f, file, f_lo, f_hi, win_size, step_size, save_file_loc, n_channels):
    ...
    window_size = samp_rate*win_size
    step_size = samp_rate*step_size
    f_lo = int(f_lo*win_size/samp_rate)
    f_hi = int(f_hi*win_size/samp_rate)
    ...
    if size > buffer_size*3:
        for chunk in xrange(0,size-3*buffer_size, buffer_size):
            if not os.path.isfile("%s%s_%i.p" % (save_file_loc, f, (chunk+buffer_size-1)/(step_size))):
                   ...
                   if not os.path.isfile("%s%s_%i.p" % (save_file_loc, f, cnt)):
                        for c in xrange(1, n_channels+1):
                            frequency[c-1,:] = (np.abs(np.fft.fft(
                                chan_sig[c-1,sub_chunk:sub_chunk+window_size]))**2)[f_lo:f_hi]
